...
leer(T,N):- random_between(1,10,T),random_between(3,10,N).
...
% La idea de esta parte del código es cortar la torta en cualquier par de trozos (P1,P2) ⊆ (P(N),P(N)), donde P1∪P2 = Torta, es decir P1 y P2 son subconjuntos disjuntos del conjunto de partes de N y se obtiene la suma de sus respectivos pagos S1 = mu1(P1)=mu1(P11)+...+mu1(P1T) y S2 = mu2(P21)+...+mu2(P2T). 
% Al hacer la consulta en prolog, tenemos 2^T soluciones

suma_de_valores(_,[],0).
suma_de_valores(N,P,S):- partir(P,[H],T), posicion(N,0,V1), posicion(N,1,V2), posicion(N,2,V3), suma_de_valores(N,T,Si),
(
  H==1 -> S is V1+Si;
  H==2 -> S is V2+Si;
  H==3 -> S is V3+Si
...
elegir(Torta,(P1,P2),(S1P1,S1P2),(S2P1,S2P2)):- u1(N1),u2(N2), append(P1,P2,Torta), suma_de_valores(N1,P1,S1P1),suma_de_valores(N1,P2,S1P2), suma_de_valores(N2,P1,S2P1),suma_de_valores(N2,P2,S2P2).
...
mejor_eleccion(Torta,(P1,P2),S1,S2):- elegir(Torta,(P1,P2),(S1P1,S1P2),(S2P1,S2P2)),
  (
    (S1P1>1,S2P2>=S2P1;((S1P1>=S1P2,S2P2>=S2P1); (S1P1>0.18,S2P2>=S2P1))) -> S1 = S1P1, S2 = S2P2; 
      ((S1P2>1,S2P1>=S2P2);(S1P2>=S1P1,S2P1>=S2P2; (S1P2>0.18,S2P1>=S2P2))) -> S1 = S1P2, S2 = S2P1
  ),!.

ciclo(T,N,L_ant1,L_ant2):- T>0,I is T-1, gen(N,[],Torta),
  write('Iteración '),write(I),write(': '),write(Torta),nl,
...
go:- leer(T,N),nl,write('T = '),write(T),write('   N = '),write(N),nl,L1=[],L2=[],ciclo(T,N,L1,L2).
